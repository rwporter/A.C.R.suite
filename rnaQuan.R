...
option_list = list(
  ...
  make_option(c("-t", "--track"), type="character", default="",
              help="(Ignored in this no-merge script; kept for compatibility) [default= %default]", metavar="character"),
...
opt_parser = OptionParser("\n\t%prog path/to/the/sample/definition/file [options]",
                          option_list=option_list,prog="rnaQuanNoMergeTags.R")
if (length(args)<1){
...
strCMD <- paste(
  "analyzeRepeats.pl rna", opt$genome,
  "-condenseGenes -count exons -noadj", opt$homer,
  "-d", paste(gsub(";"," ",sInfo[,2]),collapse = " "),
  ">", paste(strOutput,"HOMER.rawCount.txt",sep="")
)
cat("\t\t",strCMD,"\n")
...
strCMD <- paste(
  "analyzeRepeats.pl rna", opt$genome,
  "-count exons -tpm", opt$homer,
  "-d", paste(gsub(";"," ",sInfo[,2]),collapse = " "),
  ">", paste(strOutput,"HOMER.rawTPM.txt",sep="")
)
cat("\t\t",strCMD,"\n")
system(paste(strCMD,"2>/dev/null"))
...
for(i in 1:length(theta)){
  ...
  text3D(pca.x,pca.y,pca.z, rownames(Data.pca$x), col=pCol,cex=0.3, add=T)
}
a <- dev.off()
  ...
  for(i in ids){
    for(j in ids){
      ...
      if(is.null(names(tryM))){
        ...
        index <- apply(tmp,1,function(x,fit,cutZero){
          return(fit$z[sum((x[1]-fit$x)>=0),sum((x[2]-fit$y)>=0)]<cutZero)
        },f1,imageZero)
      }
...
## Step 6 removed: no mergeTag directories will be created
cat("\n\tStep 6: merge tag directories (SKIPPED in rnaQuanNoMergeTags.R)\n")
